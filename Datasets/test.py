...
# Define the URL or path to charger icon image
charger_icon_url = 'https://upload.wikimedia.org/wikipedia/commons/2/2b/EV_Charging_Station_Icon.png'

# Plot each EV station on the map
for index, row in data.iterrows():
    if pd.notna(row['Latitude']) and pd.notna(row['Longitude']):
        # Use CustomIcon for charger icon
        folium.Marker(
            ...
            icon=folium.CustomIcon(icon_image=charger_icon_url, icon_size=(30, 30)),
        ).add_to(m)
